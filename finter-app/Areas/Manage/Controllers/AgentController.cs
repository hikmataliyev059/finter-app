...
using finter_app.Areas.Manage.Helpers.Extensions;
...
public class AgentController : Controller
{
    ...
    public async Task<IActionResult> Create(CreateAgentDto agentDto)
    {
        ...
        if (!agentDto.File.ContentType.Contains("image") && agentDto.File.Length > 3_000_000)
        {
            ModelState.AddModelError("File", "File tipini ve ya olcusunu duzgun daxil edin");
            return View(agentDto);
        }
    ...
    public async Task<IActionResult> Update(UpdateAgentDto newAgentDto)
    {
        ...
        {
            if (!newAgentDto.File.ContentType.Contains("image") && newAgentDto.File.Length > 3_000_000)
            {
                ModelState.AddModelError("File", "File tipini ve ya olcusunu duzgun daxil edin");
                return View(newAgentDto);
            }
